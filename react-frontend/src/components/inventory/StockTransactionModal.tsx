...
const StockTransactionModal: React.FC<StockTransactionModalProps> = ({ isOpen, onClose, item, onSuccess }) => {
    ...
    const fetchLookups = async () => {
        try {
            const [sRes, dRes, deptRes] = await Promise.all([
                api.get('/stock/assets/lookups/suppliers'),
                api.get('/stock/assets/lookups/donors'),
                api.get('/departments')
            ]);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const payload = {
                ...
                unitPrice: price,
                remarks: remarks,
                supplier: type === 'PURCHASE' ? { id: selectedPartyId } : null,
                donor: type === 'DONATION_IN' ? { id: selectedPartyId } : null,
                department: type.includes('TRANSFER') ? { id: selectedPartyId } : null
            };
    ...
    const getIcon = () => {
        switch (type) {
            case 'PURCHASE': return <Plus className="text-emerald-500" />;
            case 'SALE': return <ShoppingCart className="text-blue-500" />;
            case 'DONATION_IN': case 'DONATION_OUT': return <Heart className="text-pink-500" />;
            case 'TRANSFER_IN': case 'TRANSFER_OUT': return <Send className="text-amber-500" />;
                ...
                <div className="space-y-6">
                    ...
                    <div className="p-5 bg-gray-50/50 dark:bg-gray-800/50 rounded-xl border border-gray-100 dark:border-gray-700/50 space-y-5">
                        ...
                        <div className="grid grid-cols-2 gap-5">
                            <div>
                                <Label className="text-[10px] font-black uppercase text-gray-500 mb-2 block tracking-widest">Quantity</Label>
                                <TextInput type="number" value={quantity} onChange={(e) => setQuantity(parseInt(e.target.value))} icon={Plus} className="shadow-sm" />
                            </div>
                            <div>
                                <Label className="text-[10px] font-black uppercase text-gray-500 mb-2 block tracking-widest">Price per Unit</Label>
                                <TextInput type="number" value={price} onChange={(e) => setPrice(parseFloat(e.target.value))} icon={DollarSign} className="shadow-sm" />
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="p-5 bg-gray-50/50 dark:bg-gray-800/50 rounded-xl border border-gray-100 dark:border-gray-700/50">
                            {type === 'PURCHASE' && (
                                <div>
                                    ...
                                    <Select value={selectedPartyId} onChange={(e) => setSelectedPartyId(e.target.value)} className="shadow-sm">
                            ...
                            {(type === 'DONATION_IN' || type === 'DONATION_OUT') && (
                                <div>
                                    <Label className="text-[10px] font-black uppercase text-gray-500 mb-2 block tracking-widest flex items-center gap-2"><Heart size={12} /> Donor / Recipient</Label>
                                    <Select value={selectedPartyId} onChange={(e) => setSelectedPartyId(e.target.value)} className="shadow-sm">
                            ...
                            {type.includes('TRANSFER') && (
                                <div>
                                    <Label className="text-[10px] font-black uppercase text-gray-500 mb-2 block tracking-widest flex items-center gap-2"><Send size={12} /> Target Department</Label>
                                    <Select value={selectedPartyId} onChange={(e) => setSelectedPartyId(e.target.value)} className="shadow-sm">
                    ...
                    {/* Remarks */}
                    <div>
                        <Label className="text-[10px] font-black uppercase text-gray-500 mb-2 block tracking-widest">Remarks</Label>
                        <Textarea value={remarks} onChange={(e) => setRemarks(e.target.value)} placeholder="Enter transaction details or notes..." rows={3} className="shadow-sm resize-none" />
                    </div>
                </div>
            </ModalBody>
            <ModalFooter>
