        ...
        websocketService.subscribe("/user/queue/messages", (msg: any) => {
          ...
          if (msg.type === "CHAT" || msg.type === "AUDIO") {
            ...
            if (!isOpen || selectedUser !== msg.sender) {
              ...
              toast.success(
                `New ${msg.type.toLowerCase()} from ${msg.sender}`,
  ...
  const initPeerConnection = () => {
    ...
    pc.ontrack = (event) => {
      ...
      stream.getAudioTracks().forEach((track) => {
        console.log(
          `Track: ${track.label}, Enabled: ${track.enabled}, ReadyState: ${track.readyState}`,
        );
      });
      ...
      if (remoteAudioRef.current) {
        ...
        if (playPromise !== undefined) {
          playPromise.catch((error) => {
            console.error(
              "Auto-play was prevented. Adding manual play trigger.",
              error,
            );
  ...
  const fetchHistory = async () => {
    try {
      const response = await axios.get(
        `/api/chat/history/user/${currentUser.username}`,
        {
      ...
      {!isOpen && (
        <div className="relative group">
          ...
          <button
            ...
            className="p-4 rounded-full shadow-2xl transition-all duration-500 transform hover:scale-110 active:scale-95 flex items-center justify-center bg-blue-600 text-white hover:bg-blue-700"
          >
        ...
        <div className="mb-4 w-[92vw] sm:w-[24rem] md:w-[26rem] h-[38rem] max-h-[85vh] bg-white dark:bg-gray-800 rounded-md shadow-[0_25px_60px_rgba(0,0,0,0.3)] border border-gray-100 dark:border-gray-700 flex flex-col overflow-hidden animate-in slide-in-from-bottom-8 duration-500">
          ...
          >
            <div className="flex items-center gap-3 relative z-10">
              ...
              <div>
                <h3 className="font-black text-sm">
                  {selectedUser
                    ? callActive
                      ? "In Call with " + selectedUser
                      : isCalling
                        ? "Calling " + selectedUser + "..."
                ...
                <div className="flex gap-2">
                  ...
                  {selectedUser && !callActive && (
                    <button
                      onClick={() => setSelectedUser(null)}
                      className="text-[10px] font-black bg-white/20 px-2 py-0.5 rounded uppercase"
                    >
                      Public
                    </button>
                  )}
            ...
            <div className="flex items-center gap-2 relative z-10">
              {selectedUser && !callActive && !isCalling && (
                <button
                  onClick={startCall}
                  className="p-2.5 bg-white/20 hover:bg-white/30 rounded-md transition-all animate-pulse"
                >
                  <Phone size={20} />
                </button>
              )}
              {(callActive || isCalling) && (
                <button
                  onClick={stopCall}
                  className="p-2.5 bg-red-500 hover:bg-red-600 rounded-md transition-all"
                >
                  <PhoneOff size={20} />
                </button>
              )}
              ...
              <div className="space-y-4 overflow-y-auto flex-1 pr-2 custom-scrollbar">
                {onlineUsers.map((user, idx) => (
                  <div
                    ...
                    className="flex items-center gap-4 p-3 hover:bg-blue-50 rounded-md cursor-pointer transition-all"
                  >
                    <Avatar
                      placeholderInitials={user.substring(0, 2).toUpperCase()}
                      rounded
                      size="sm"
                    />
          ...
          <div className="flex-1 p-6 overflow-y-auto bg-gray-50/50 dark:bg-gray-900/20 space-y-4 custom-scrollbar">
            {filteredMessages.map((msg, idx) => {
              ...
              if (msg.type !== "CHAT" && msg.type !== "AUDIO") return null;
              ...
              {isRecording ? (
                ...
                <>
                  <input
                    ...
                    className="flex-1 bg-gray-50 dark:bg-gray-700 border-none rounded-md px-5 py-3 text-sm focus:ring-2 focus:ring-blue-500"
                  />
                  <button
                    type="submit"
                    disabled={!message.trim()}
                    className="p-3 bg-blue-600 hover:bg-blue-700 text-white rounded-md shadow-xl active:scale-95 transition-all"
                  >
                    <Send size={24} />
                  </button>
                </>
