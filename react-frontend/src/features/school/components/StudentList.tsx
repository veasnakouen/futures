...
export default function StudentList() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => schoolService.getStudents(page, size).then((res) => res.data),
  });

  const deleteMutation = useMutation({
    mutationFn: (id: number) => schoolService.deleteStudent(id),
    onSuccess: () => {
                ...
                data?.content?.map((student: StudentDto) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <span className="px-2.5 py-1 bg-green-50 dark:bg-green-900/30 text-green-600 dark:text-green-400 text-xs font-medium rounded-lg">
