...
export default function LabOrderList() {
  ...
  const filteredData = data?.content?.filter((p: LabOrderDto) => {
    const matchesSearch = p.testName?.toLowerCase().includes(searchTerm.toLowerCase()) || 
                          p.loincCode?.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
        <div>
          <h2 className="text-xl font-bold text-gray-900 dark:text-white">Lab Orders</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage laboratory tests and results</p>
        </div>
          ...
          <table className="w-full text-left text-sm text-gray-500 dark:text-gray-400">
            <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-900/50 dark:text-gray-400">
              <tr>
                ...
                <th className="px-6 py-4 font-bold">LOINC Code</th>
                <th className="px-6 py-4 font-bold">Status</th>
                <th className="px-6 py-4 font-bold">Result Value</th>
                <th className="px-6 py-4 font-bold text-right">Actions</th>
              </tr>
            </thead>
            <tbody>
              {isLoading ? (
                <tr><td colSpan={5} className="px-6 py-8 text-center text-gray-500">Loading lab orders...</td></tr>
              ) : filteredData.length === 0 ? (
                <tr><td colSpan={5} className="px-6 py-12 text-center">No lab orders found.</td></tr>
              ) : (
                filteredData.map((item: LabOrderDto) => (
                  <tr key={item.id} className="bg-white dark:bg-gray-800 border-b border-gray-50 dark:border-gray-700/50 hover:bg-gray-50 dark:hover:bg-gray-700/25 transition-colors">
                    <td className="px-6 py-4 font-medium text-gray-900 dark:text-white">{item.testName}</td>
                    <td className="px-6 py-4">{item.loincCode}</td>
                    ...
                    <td className="px-6 py-4">{item.resultValue || "N/A"}</td>
