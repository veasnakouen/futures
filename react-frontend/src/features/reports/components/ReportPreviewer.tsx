...
const formatReportTitle = (rawName: string): string => {
  if (!rawName) return "Standard Ledger Report";

  const knownTitles: Record<string, string> = {
    EmployeeEvaluationReport: "Employee Evaluation Report",
    EMPLOYEEEVALUATIONREPORT: "Employee Evaluation Report",
    PlacementSummaryReport: "Placement Summary Report",
    PLACEMENTSUMMARYREPORT: "Placement Summary Report",
    ClientSummaryReport: "Client Summary Report",
    AttendanceReport: "Attendance Summary Report",
    EmployeeSeekerListreport: "Employee Seeker List Report",
    EmployersReport: "Employers Report",
    JobVancancyAvailable: "Job Vacancies Available Report",
    BeneficiariesReport: "Beneficiaries Report",
    Businesssetupmonitoiring: "Business Setup Monitoring Report",
    BusinessSetUpReport: "Business Set Up Report",
  };

  if (knownTitles[rawName]) return knownTitles[rawName];
...
const ReportPreviewer: React.FC<ReportPreviewerProps> = ({
  ...
  return (
    <div className="flex-1 flex flex-col min-h-[700px] -z-10">
      <div className="flex-1 bg-white/90 dark:bg-gray-800/90 backdrop-blur-md border-none shadow-xl overflow-hidden flex flex-col p-0 rounded-2xl">
        <div className="p-6 border-b border-gray-100/60 dark:border-gray-800/30 bg-gray-50/50 dark:bg-gray-700/30 flex justify-between items-center">
          <div className="flex items-center gap-4">
            <Badge
              color="info"
              className="rounded-xl px-4 py-1.5 font-black uppercase text-[10px] tracking-widest shadow-sm"
            >
              Preview Node
            </Badge>
            <span className="text-xs font-black dark:text-gray-300 text-gray-700 uppercase tracking-widest">
              {formatReportTitle(selectedReport)}
            </span>
            {totalRecords > 0 && (
              <Badge
                color="success"
                className="ml-3 rounded-xl font-black text-[9px] uppercase tracking-widest px-3 py-1"
              >
